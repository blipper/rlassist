...
class AC_Network():
    ...
    def __init__(self, args, vocab_size, num_actions, seed):
        self.args = args
    ...
    def create_model(self):
        class PolicyValueError(tf.keras.losses.Loss):
            def call(self, y_true, y_pred):
                actions_input = tf.keras.layers.Input()
                one_hot_layer = tf.one_hot(actions_input, self.num_actions)
                responsible_outputs = tf.keras.layers.Multiply(policy_layer, one_hot_layer)

                self.value_loss = 0.5 * tf.reduce_sum(input_tensor=tf.square(y_true - tf.reshape(y_pred, [-1])))
        ...
        inputs = tf.keras.Input(shape=(self.vocab_size,), name="State")
        compiler_inputs = tf.keras.Input(shape=(self.vocab_size,), name="CompilerErrors")
        concat_inputs = tf.keras.layers.Concatenate(name = "StateWithCompilerErrors")([inputs, compiler_inputs])
        embedding_layer = tf.keras.layers.Embedding(self.vocab_size, self.embedding_size, name="EncodedState")(concat_inputs)
        tokenized_state = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(128, dropout = dropout, return_sequences=True))(embedding_layer)
        tokenized_state =  tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(128, dropout = dropout, return_sequences=True))(tokenized_state)
        mean_pooling_layer = tf.keras.layers.GlobalAveragePooling1D(name="MeanPoolingLayer")(tokenized_state)
        policy_layer = tf.keras.layers.Dense(self.num_actions, activation = tf.nn.softmax, name="PolicyLayer")(mean_pooling_layer)
        value_layer = tf.keras.layers.Dense(1, activation=None, name="ValueLayer")(mean_pooling_layer)
        policy_layer_clipped = tf.clip_by_value(policy_layer, 1e-10, 1., name="ClippedPolicyLayer")
        model = tf.keras.Model(inputs=[inputs, compiler_inputs], outputs=[policy_layer_clipped,value_layer])
        model.compile(loss=PolicyValueError(), optimizer=tf.keras.optimizers.Adam(learning_rate=1e-4))
        print(model.summary())
...
class Worker():
    ...
    def __init__(self, env, args, name, vocab_size, model_path, global_episodes, T, book_keeper, seed, master_network):
        ...
        self.local_AC = AC_Network(args, vocab_size, env.num_actions, seed=seed*2+1)
    ...
    def show_final_eval_summary(self, which):
        self.book_keeper.show_eval_summary(Worker.global_episodes, which)

    def show_final_train_summary(self):
    ...
    def train(self, rollout, gamma, bootstrap_value):
        ...
        print(len(prog_batch))
        print(len(actions))

        self.rewards_plus = np.asarray(rewards.tolist() + [bootstrap_value])
        discounted_rewards = discount(self.rewards_plus, gamma)[:-1]
        self.value_plus = np.asarray(values.tolist() + [bootstrap_value])
        returns = rewards + gamma * self.value_plus[1:]
        advantages =  returns - self.value_plus[:-1]
        advantages = discount(advantages, gamma)

        acts_and_advs = np.concatenate([actions[:, None], advantages[:, None]], axis=-1)
        print(acts_and_advs)
        value_loss, policy_loss = self.local_AC.model.train_on_batch([prog_batch], [acts_and_advs, returns])
        ...
        #                                     ], feed_dict=feed_dict)
        return value_loss / rollout.size, policy_loss / rollout.size
        ...
        with coord.stop_on_exception():
            while not coord.should_stop() and start < end:
                try:
                    ...
                    start += 1
                self.local_AC.model.set_weights(self.master_network.model.get_weights())
                ...
                while episode_step_count < max_episode_length:
                    ...
                    action_dist, state_value = self.local_AC.model.predict(prog_batch)
        ...
        with coord.stop_on_exception():
            while not coord.should_stop() and Worker.global_episodes <= Worker.max_global_episodes:
                ...
                while episode_step_count < max_episode_length:
                    ...
                    action_dist, v = self.local_AC.model.predict(np.transpose(prog_batch))
                    ...
                    if episode_buffer.size == self.batch_size and halt != True and episode_step_count != max_episode_length - 1:
                        ...
                        v_l, p_l, e_l, g_n, v_n = self.train(episode_buffer, gamma, v1)
                ...
                if episode_buffer.size != 0:
                    v_l, p_l, e_l, g_n, v_n = self.train(episode_buffer, gamma, 0.0)
                ...
                if self.name == 'worker_0' and self.is_it_time_for(args.evaluation_frequency, which='eval'):
                    self.save_model_and_start_eval(self.model)
...
if __name__ == '__main__':
    ...
    #print 'Left %d spare cores, running training only on the following cores:' % spare_cores, '\n'
    ...
    def do_eval(env, args, which='test'):
        try:
            ...
            for i, worker in enumerate(workers):
                ...
                def worker_work(): return worker.evaluate(args.max_epLength, eval_coord, which, start, end)
            ...
            workers[0].show_final_eval_summary(which)
    ...
    with tf.device("/cpu:0"):
        ...
        master_network = AC_Network(args, vocab_size, num_actions, seed=args.seed*2)
        ...
        for i in range(num_workers):
            workers.append(Worker(get_env(dataset, -1.0 / args.max_epLength, args, action_names, i == 0, args.seed + i + 1, train_data_size,
                                          GE_code_ids=guided_exploration_code_ids), args, i, vocab_size, ckpt_dir,
                                  global_episodes, best_ckpt, book_keeper, args.seed + i + 100,master_network))
        ...
        if args.evaluate_at == 0:
            try:
                ...
                if args.resume:
                    ...
                    self.model.load_model(os.path.join(ckpt_dir, 'model-%d' % best_ckpt))

            # Start the "work" process for each worker in a separate thread.
                worker_threads = []
                for worker in workers:
                    def worker_work(): return worker.work(max_episode_length, gamma, coord)
                    t = threading.Thread(target=(worker_work))
                    t.start()
                    sleep(0.5)
                    worker_threads.append(t)
                ...
                workers[0].save_model_and_start_eval(master_network.model)
            except KeyboardInterrupt:
            ...
            try:
                self.model.load_model(os.path.join(ckpt_dir, 'model-%d' % args.evaluate_at))
            except:
                self.model.load_model(os.path.join(ckpt_dir, 'epoch-ckpts', 'model-%d' % args.evaluate_at))
            if args.do_not_reject_edits_during_eval:
                print('----- evaluating WITHOUT edit rejection ------')
            else:
                print('----- evaluating WITH edit rejection ------')

            do_eval(temp_env, args, which=args.eval_which)
